...
def consultar (tree):
    ...
    # Removendo os dados do Treeview
        ...
        # Caso o Usuário tenha preenchido o campo TIPO e FABRICANTE
            ...
            curFerramentas.execute("""SELECT * FROM tabelaFerramentas WHERE tipo=? and fabricante=?""", [tipo, fabricante])
            resultado = curFerramentas.fetchall()
            for row in range(0, len(resultado)):
...
e_tipo = Entry(font=("Verdana 9 bold"), bg=corBranca, highlightbackground=corBorda, fg=corLetra, justify="left", width= 41)
...
e_fabricante = Entry(font=("Verdana 9 bold"), bg=corBranca, highlightbackground=corBorda, fg=corLetra, justify="left", width= 35)
...
# BOTÕES ###########################################################################################################################################################
    # Botão Cancelar 
btn_voltar = Button(master=janConsultaFerramenta, text="VOLTAR", font=("Verdana 9 bold"), highlightbackground=corBorda, fg=corLetra,  height=2, width=15)
btn_voltar.place(x=600, y=130)

    # Botão Limpar Busca
btn_limparBuscar = Button(master=janConsultaFerramenta, command=lambda: limparBusca(tabela_dados), text="LIMPAR BUSCA", font=("Verdana 9 bold"), highlightbackground="#FFE19C", fg=corLetra, height=2, width=15)
btn_limparBuscar.place(x=600, y=190)

    # Botão Consultar Item Específico
btn_consultar = Button(master=janConsultaFerramenta, command=lambda: consultar(tabela_dados), text="CONSULTAR", font=("Verdana 9 bold"), highlightbackground=corVerde, fg=corLetra, height=2, width=15)
