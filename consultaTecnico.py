...
def consultar (tree):
    ...
    # Removendo os dados do Treeview
        ...
        # Caso o Usuário tenha preenchido o campo NOME, TURNO e CPC
            ...
            cpf = int(cpf)
            curTecnicos.execute("""SELECT * FROM tabelaTecnicos WHERE nome=? and turno=? and cpf=?""", [nome, turno, cpf])
            resultado = curTecnicos.fetchall()
            for row in range(0, len(resultado)):
...
e_nome = Entry(font=("Verdana 9 bold"), bg=corBranca, highlightbackground=corBorda, fg=corLetra, justify="left", width= 40)
e_nome.place(x=90, y=120) 

    # Campo Tipo
l_turno = Label(text='TURNO', bg=corCinza, fg=corLetra, font=("Verdana 12 bold"))
l_turno.place(x=25, y=160)
e_turno = Entry(font=("Verdana 9 bold"), bg=corBranca, highlightbackground=corBorda, fg=corLetra, justify="left", width= 40)
e_turno.place(x=90, y=160)

    # Campo Fabricante
l_cpf = Label(text='CPF', bg=corCinza, fg=corLetra, font=("Verdana 12 bold"))
l_cpf.place(x=25, y=200)
e_cpf= Entry(font=("Verdana 9 bold"), bg=corBranca, highlightbackground=corBorda, fg=corLetra, justify="left", width= 40)
...
# BOTÕES ###########################################################################################################################################################
    # Botão Cancelar 
btn_voltar = Button(master=janConsultaTecnico, text="VOLTAR", font=("Verdana 9 bold"), highlightbackground=corBorda, fg=corLetra,  height=2, width=15)
btn_voltar.place(x=450, y=127)

    # Botão Limpar Busca
btn_limparBuscar = Button(master=janConsultaTecnico, command=lambda: limparBusca(tabela_dados), text="LIMPAR BUSCA", font=("Verdana 9 bold"), highlightbackground="#FFE19C", fg=corLetra, height=2, width=15)
btn_limparBuscar.place(x=450, y=187)

    # Botão Consultar Item Específico
btn_consultar = Button(master=janConsultaTecnico, command=lambda: consultar(tabela_dados), text="CONSULTAR", font=("Verdana 9 bold"), highlightbackground=corVerde, fg=corLetra, height=2, width=15)
